...
bool MagicMusicKeyboard::processKey(uint8_t btCode, bool isPressed)
{
	...
	if (!(1 <= btCode && btCode <= BTKC_GUI_LEFT))
	{
		//do nothing
		return false;
	}

	else if (btCode == BTKC_GUI_LEFT)
	{
		//reset key (backspace)
		if (isPressed)
		{
...
{
  usbMIDI.sendNoteOn(note, NOTE_VELOCITY, MIDIKANAL); 
...
{
  usbMIDI.sendNoteOff(note, NOTE_VELOCITY, MIDIKANAL); 
